        ...
        with st.expander(f"Sheet: {sh}", expanded=True):
            ...
            for col in df.columns:
                ...
                if errors:
                    ...
                    # Show first 10 errors to save space
                    err_df = pd.DataFrame(errors, columns=["Row", "Value", "Issue"])
                    st.dataframe(err_df.head(10), use_container_width=True, hide_index=True)
                    if len(errors) > 10:
...
with tab2:
    ...
    summary_cols = ["Campus", "City", "Region", "Duration", "First Year Fee", "Cost of living", "Minimum Deposit", "Scholarship"]
    
    sel_sheet = st.selectbox("Select Sheet for Summary", list(sheets.keys()))
    df_sum = sheets[sel_sheet]
    
    cols = st.columns(2)
    for idx, target_col in enumerate(summary_cols):
        # Find actual column name in df
        actual_col = find_column(df_sum, [normalize(target_col)])
        
        with cols[idx % 2]:
            if actual_col:
                unique_vals = df_sum[actual_col].unique()
                st.selectbox(
                    f"Unique values in `{actual_col}` ({len(unique_vals)})", 
                    options=unique_vals,
                    key=f"sum_{sel_sheet}_{actual_col}"
                )
            else:
...
with tab3:
    ...
    if st.button("🚀 Run Auto-Fix (Apply Rules + User Inputs)", type="primary"):
        ...
        for sh, df_orig in sheets.items():
            ...
            uni_col = find_column(df_fix, ["university"])
            if uni_col and user_uni_name:
                df_fix[uni_col] = user_uni_name
                
            link_col = find_column(df_fix, ["universitylink", "link"])
